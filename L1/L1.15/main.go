...
func (j *justString) someFunc() {
	...
	// Clone чтобы не сохранять ссылку на большую исходную строку
	j.s = strings.Clone(string([]rune(v)[:3])) // Используем срез рун для корректного захвата N символов из строки
